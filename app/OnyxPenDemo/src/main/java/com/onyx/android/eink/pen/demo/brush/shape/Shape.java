...
public class Shape {
    ...
    protected ShapeCreateArgs shapeCreateArgs;
    ...
    public Shape setShapeCreateArgs(ShapeCreateArgs shapeCreateArgs) {
        this.shapeCreateArgs = shapeCreateArgs;
        return this;
    }

    public SplitShapeResult split(EraseBean eraseBean) {
        return ShapeSplitter.split(this, eraseBean);
    }
    ...
    public boolean fastHitTest(float x, float y, float radius) {
        ...
        return boundingRect.contains(x, y)
                || Math.abs(boundingRect.left - x) <= radius
                || Math.abs(boundingRect.right - x) <= radius
                || Math.abs(boundingRect.top - y) <= radius
                || Math.abs(boundingRect.bottom - y) <= radius;
    }

    public boolean hitTestSegment(float x1, float y1, float x2, float y2) {
        return hitTest(x1, y1, 1f) || hitTest(x2, y2, 1f);
    }
