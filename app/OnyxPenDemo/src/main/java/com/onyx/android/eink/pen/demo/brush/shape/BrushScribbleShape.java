...
public class BrushScribbleShape extends Shape {
    ...
    private void renderFountainV2(RendererHelper.RenderContext renderContext,
        ...
        float displayScale = shapeCreateArgs != null ? shapeCreateArgs.getDisplayScale() : 1.0f;
        Float pressureSensitivity = shapeCreateArgs != null
                ? shapeCreateArgs.getPressureSensitivity() : null;
        NeoPen pen = FountainShapes.INSTANCE.createNeoPenV2(getRenderStrokeWidth(),
                NeoFountainPenWrapper.MIN_FOUNTAIN_PEN_WIDTH,
                1.0f, 1.0f, 1.0f, displayScale, pressureSensitivity, true, null);
        if (pen == null) {
            return;
        }
    ...
    private List<TouchPoint> copyAndNormalizePressure(List<TouchPoint> points) {
        ...
        float maxPressure = EpdController.MAX_TOUCH_PRESSURE;
        for (TouchPoint point : points) {
            if (point == null) {
                continue;
            }
            TouchPoint copy = new TouchPoint(point);
            copy.pressure = copy.getPressure() / maxPressure;
